...
# worktree 격리(병렬 전제가 cwd 안 나눠씀) + 재시작 이어가기
python3 -m teamlead --real --base-repo /path/to/git/repo --goal "..."
python3 -m teamlead --real --resume --goal "..."   # 같은 goal 의 handoff.json 이어받기
```
...
- ~~`--real` 에 `--base-repo` 인자 노출~~ ✅ 완료 (claude/codex 어댑터에 배선, capabilities 정직 반영 — worktree 격리 실사용 가능)
